...
def user(request):
    ...
    class UserFactory(object):
        ...
        def add_to_team(self, user, team):
            re = requests.post(
                api_v0("teams/%s/users" % team), json={"name": user}
            )
            assert re.status_code == 201

        def add_to_roster(self, user, team, roster):
            re = requests.post(
                api_v0("teams/%s/rosters/%s/users" % (team, roster)),
                json={"name": user},
            )
            assert re.status_code == 201
...
def team(request, user, service):
    ...
    class TeamFactory(object):
        ...
        def create(self):
            ...
            re = requests.post(
                api_v0("teams"),
                json={"name": name, "scheduling_timezone": "utc"},
            )
            assert re.status_code in [201, 422]
        ...
        def cleanup(self):
            ...
            if self.created_ids:
                self.cursor.execute(
                    "DELETE FROM team WHERE id IN %s", (self.created_ids,)
                )
...
def roster(request, team):
    ...
    class RosterFactory(object):
        ...
        def create(self, team_name):
            roster_name = "_".join(
                [self.prefix, "roster", str(len(self.created))]
            )
            re = requests.post(
                api_v0("teams/%s/rosters" % team_name),
...
def schedule(roster, role):
    ...
    class ScheduleFactory(object):
        ...
        def create(self, team_name, roster_name, json):
            re = requests.post(
                api_v0(
                    "teams/%s/rosters/%s/schedules" % (team_name, roster_name)
                ),
                json=json,
            )
            assert re.status_code == 201
...
def event(team, role):
    ...
    class EventFactory(object):
        ...
        def link(self, ids):
            ...
            link_id = uuid4().hex
            cursor.execute(
                "UPDATE `event` SET `link_id` = %s WHERE `id` IN %s",
                (link_id, ids),
            )
            connection.commit()
            cursor.close()
        ...
        def cleanup(self):
            ...
            for t in self.teams:
                re = requests.get(
                    api_v0("events?include_subscribed=false"),
                    params={"team": t},
                )
                for ev in re.json():
...
def service(request):
    class ServiceFactory(object):
        ...
        def associate_team(self, service_name, team_name):
            requests.post(
                api_v0("teams/%s/services" % team_name),
        ...
        def cleanup(self):
            for team_name, service_name in self.mappings:
                requests.delete(
                    api_v0("teams/%s/services/%s" % (team_name, service_name))
                )
