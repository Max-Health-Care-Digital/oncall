...
def test_audit(team, user, role, roster, event):
    ...
    requests.put(
        api_v0("teams/" + team_name_2),
        json={
            "email": "foo",
            "slack_channel": "#bar",
            "slack_channel_notifications": "#bar-alerts",
        },
    )
    ...
    requests.put(
        api_v0("teams/%s/rosters/foo" % team_name), json={"name": roster_name}
    )
    # roster user actions
    requests.post(
    ...
    re = requests.put(
        ...
        json={"start": ev_start + 5, "end": ev_end - 5},
    )
    assert re.status_code == 200
    # swap events
    requests.post(
    ...
    start = int(time.time())
    requests.post(
        api_v0("teams/%s/admins" % team_name), json={"name": user_name}
    )
