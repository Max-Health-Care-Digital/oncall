...
def test_invalid_event_swap(team, user, role, event):
    ...
    re = requests.post(
        api_v0("events/swap"),
        json={"events": [{"id": ev1, "linked": False}, {"linked": False}]},
    )
    assert re.status_code == 400
...
def test_events_link(team, user, role):
    ...
    assert re.status_code == 200
    evs = [
        requests.get(api_v0("events/%d" % eid)).status_code == 404
        for eid in ev_ids
    ]
    assert all(evs)
