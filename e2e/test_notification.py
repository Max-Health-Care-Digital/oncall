...
def test_notification_settings(team, user, role):
    ...
    re = requests.post(
        api_v0("users/%s/notifications" % user_name), json=notification
    )
...
def test_notify_on_ev_actions(user, team, role, event):
    ...
    for type_name in (EVENT_CREATED, EVENT_EDITED, EVENT_DELETED):
        ...
        re = requests.post(
            api_v0("users/%s/notifications" % user_name_2), json=notification
        )
        assert re.status_code == 201
        ...
        re = requests.post(
            api_v0("users/%s/notifications" % user_name_3),
        ...
        re = requests.post(
            api_v0("users/%s/notifications" % user_name_2),
            json=new_notification,
        )
        assert re.status_code == 201
    ...
    notifications = get_notifications(
        [user_name, user_name_2, user_name_3], EVENT_CREATED
    )
    ...
    notifications = get_notifications(
        [user_name, user_name_2, user_name_3], EVENT_EDITED
    )
    assert len(notifications) == 3
    assert {n["user"] for n in notifications} == {
        user_name,
        user_name_2,
        user_name_3,
    }
    ...
    notifications = get_notifications(
        [user_name, user_name_2, user_name_3], EVENT_DELETED
    )
...
def test_notify_on_override(user, role, team, event):
    ...
    notifications = get_notifications(
        [user_name, user_name_2], EVENT_SUBSTITUTED
    )
    assert len(notifications) == 2
    assert {n["user"] for n in notifications} == {user_name, user_name_2}
