...
def test_v0_pin_team(user, team):
    ...
    # Test pinning teams
    re = requests.post(
        api_v0("users/%s/pinned_teams" % user_name), json={"team": team_name}
    )
    assert re.status_code == 201
    re = requests.post(
        api_v0("users/%s/pinned_teams" % user_name), json={"team": team_name_2}
    )
    assert re.status_code == 201
    ...
    re = requests.delete(
        api_v0("users/%s/pinned_teams/%s" % (user_name, team_name))
    )
    assert re.status_code == 200
...
def test_api_v0_pin_invalid(user, team):
    ...
    re = requests.post(
        api_v0("users/%s/pinned_teams" % user_name),
    ...
    re = requests.post(
        api_v0("users/%s/pinned_teams" % "nonexistent-user-foobar"),
        json={"team": team_name},
    )
    assert re.status_code == 422
