...
def test_api_v0_populate_new(user, team, roster, role, schedule):
    ...
    re = requests.get(
        api_v0(
            "schedules/%s/preview?team__eq=%s&start=%s&teamName=%s&end__ge=%s&start__lt=%s"
            % (schedule_id, team__eq, start, team_name, end__ge, start__lt)
        )
    )
    ...
    re = requests.post(
        api_v0("schedules/%s/populate" % schedule_id), json={"start": start}
    )
    assert re.status_code == 200
...
def test_v0_populate_vacation_propagate(
    user, team, roster, role, schedule, event
):
    user_name = user.create()
    user_name_2 = user.create()
    team_name = team.create()
    ...
    # Populate for team 1
    re = requests.post(
        api_v0("schedules/%s/populate" % schedule_id),
    ...
    re = requests.get(
        api_v0("events?team=%s&include_subscribed=false" % team_name)
    )
    assert re.status_code == 200
    events = re.json()
    assert len(events) == 2
...
def test_api_v0_populate_over(user, team, roster, role, schedule):
    ...
    now = time.time()
    re = requests.post(
        api_v0("schedules/%s/populate" % schedule_id), json={"start": now}
    )
    assert re.status_code == 200
    ...
    re = requests.put(
        api_v0("schedules/%s" % schedule_id), json={"events": new_events}
    )
    assert re.status_code == 200
...
def test_api_v0_populate_invalid(user, team, roster, role, schedule):
    ...
    re = requests.post(
        api_v0("schedules/%s/populate" % schedule_id),
        json={"start": time.time() - 14 * 24 * 3600},
    )
    ...
    re = requests.post(
        api_v0("schedules/%s/populate" % schedule_id),
        json={"start": time.time() + 21 * 3600 * 24},
    )
    assert re.status_code == 200
    re = requests.get(api_v0("schedules/%s" % schedule_id))
    assert re.status_code == 200
...
def test_api_v0_populate_subscription(
    user, team, roster, role, schedule, event
):
    user_name = user.create()
    user_name_2 = user.create()
