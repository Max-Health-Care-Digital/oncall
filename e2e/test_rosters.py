...
def test_api_v0_rosters(team):
    ...
    def clean_up():
        requests.delete(
            api_v0("teams/%s/rosters/%s" % (team_name, roster_name))
        )
        requests.delete(
            api_v0("teams/%s/rosters/%s" % (team_name, roster_name2))
        )
    ...
    # test create rosters
    re = requests.post(
        api_v0("teams/%s/rosters" % team_name), json={"name": roster_name}
    )
    assert re.status_code == 201
    re = requests.post(
        api_v0("teams/%s/rosters" % team_name), json={"name": roster_name2}
    )
    assert re.status_code == 201
    ...
    re = requests.get(
        api_v0(
            "teams/%s/rosters?name__contains=%s&name__startswith=%s&name__endswith=%s"
    ...
    assert re.json() == {
        ...
        "description": "roster '%s' already existed for team '%s'"
        % (roster_name2, team_name),
    }
...
def test_api_v0_rosters_users(team, roster, user):
    ...
    re = requests.delete(
        api_v0(
            "teams/%s/rosters/%s/users/%s" % (team_name, roster_name, user_name)
        )
    )
    assert re.status_code == 200
    re = requests.get(
...
def test_api_v0_rotation(team, user, roster):
    ...
    assert re.status_code == 201
    requests.post(
        api_v0("teams/%s/rosters/%s/users" % (team_name, roster_name_2)),
        json={"name": user_name, "in_rotation": False},
    )
    ...
    re = requests.get(
        api_v0(
            "teams/%s/rosters/%s/users?in_rotation=1" % (team_name, roster_name)
        )
    )
    assert re.status_code == 200
    users = re.json()
    ...
    re = requests.get(
        api_v0(
            "teams/%s/rosters/%s/users?in_rotation=0"
            % (team_name, roster_name_2)
        )
    )
    assert re.status_code == 200
    users = re.json()
