...
def test_api_v0_schedules(team, roster, role):
    ...
    re = requests.post(
        api_v0("teams/%s/rosters/%s/schedules" % (team_name, roster_name)),
        json={"role": role_name, "events": events, "advanced_mode": 1},
    )
    assert re.status_code == 400

    # test create schedule
    events = [
        {"start": tuesday9am, "duration": 12 * HOUR},
    ...
    updated_events = [
        ...
        {"start": 150, "duration": 200},
    ]
    # verify schedule updates properly
    re = requests.put(
        api_v0("schedules/" + schedule_id),
        json={
            "role": role_name_2,
            "team": team_name_2,
    ...
    assert re.status_code == 200
    re = requests.get(
        api_v0("teams/%s/rosters/%s/schedules" % (team_name_2, roster_name_2))
    )
    assert re.status_code == 200
    data = re.json()
    assert len(data) == 1
    ...
    re = requests.put(
        api_v0("schedules/" + schedule_id),
        json={"team": team_name, "roster": roster_name},
    )
    assert re.status_code == 200
...
def test_api_v0_advanced_schedule(team, roster, role, schedule):
    ...
    re = requests.put(
        api_v0("schedules/%d" % schedule_id),
        json={
            "events": [
                {"start": 0, "duration": 100},
    ...
    assert re.status_code == 400
    re = requests.put(
        api_v0("schedules/%d" % schedule_id), json={"advanced_mode": 0}
    )
    assert re.status_code == 400
...
def test_api_v0_invalid_schedule_event(team, roster, role, schedule):
    ...
    re = requests.post(
        ...
        json={
            "role": role_name,
            "events": [{"duration": 100}, {"start": 150, "duration": 300}],
            "advanced_mode": 1,
        },
    )
    assert re.status_code == 400
