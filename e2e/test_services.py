...
def test_teams_services_mappings(team, service):
    ...
    re = requests.post(
        api_v0("teams/%s/services" % team_name), json={"name": service_name}
    )
    assert re.status_code == 201
    ...
    assert (team_name, service_name) in set(
        (item["team"], item["service"]) for item in team_services
    )
...
def test_api_v0_service_override_number(team, user, role, event, service):
    ...
    re = requests.get(
        api_v0("services/%s/oncall/%s" % (service_name, "primary"))
    )
    assert re.status_code == 200
    results = re.json()
    assert results[0]["start"] == start
