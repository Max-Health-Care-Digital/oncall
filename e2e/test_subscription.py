...
def test_api_v0_team_subscription(team, role):
    ...
    re = requests.post(
        api_v0("teams/%s/subscriptions" % team_name),
        json={"role": role_name, "subscription": team_name_2},
    )
    assert re.status_code == 201
    re = requests.post(
        api_v0("teams/%s/subscriptions" % team_name),
        json={"role": role_name, "subscription": team_name_3},
    )
    assert re.status_code == 201
    ...
    re = requests.delete(
        api_v0(
            "teams/%s/subscriptions/%s/%s" % (team_name, team_name_3, role_name)
        )
    )
    assert re.status_code == 200
...
def test_v0_subscription_summary(user, role, team, event):
    ...
    re = requests.post(
        api_v0("teams/%s/subscriptions" % team_name),
        json={"role": role_name_2, "subscription": team_name_2},
    )
    assert re.status_code == 201
