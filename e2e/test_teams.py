...
def test_api_v0_create_team(team):
    ...
    re = requests.post(
        api_v0("teams"), json={"name": team_name, "scheduling_timezone": "utc"}
    )
...
def test_api_v0_create_team_with_space(team):
    ...
    re = requests.post(
        api_v0("teams"),
        json={"name": team_name_with_space, "scheduling_timezone": "utc"},
    )
...
def test_api_v0_create_invalid_team(team):
    ...
    re = requests.put(
        api_v0("teams/%s" % team_name), json={"name": invalid_name}
    )
    assert re.status_code == 400
...
def test_api_v0_get_team(team, role, roster, schedule):
    ...
    re = requests.get(
        api_v0(
            "teams/%s?fields=users&fields=services&fields=admins" % team_name
        )
    )
    assert re.status_code == 200
    team = re.json()
...
def test_api_v0_team_admin(team, user):
    ...
    # test create admin
    re = requests.post(
        api_v0("teams/%s/admins" % team_name), json={"name": admin_user}
    )
    assert re.status_code == 201
    # verify result
    re = requests.get(api_v0("teams/%s/admins" % team_name))
...
def test_api_v0_team_members(team, user, roster):
    ...
    re = requests.post(
        api_v0("teams/%s/users") % team_name, json={"name": none_exist_user}
    )
    ...
    re = requests.post(
        api_v0("teams/%s/users") % team_name, json={"name": user_name}
    )
    assert re.status_code == 201
    # verify team members
    re = requests.get(api_v0("teams/%s/users" % team_name))
    ...
    assert isinstance(users, list)
    users = set(
        item["user"] for item in team_users if item["team"] == team_name
    )
    assert set(users) == set([user_name, test_user])
    ...
    assert re.json() == {
        "title": "IntegrityError",
        "description": 'user name "%s" is already in team %s'
        % (user_name, team_name),
    }
    ...
    # test create admin
    re = requests.post(
        api_v0("teams/%s/admins" % team_name), json={"name": user_name_2}
    )
    assert re.status_code == 201
    re = requests.get(api_v0("teams/%s/users" % team_name))
    assert re.status_code == 200
    ...
    re = requests.post(
        api_v0("teams/%s/rosters/%s/users" % (team_name, roster_name)),
    ...
    re = requests.post(
        api_v0("teams/%s/admins" % team_name), json={"name": user_name_3}
    )
    assert re.status_code == 201
    re = requests.delete(
        api_v0("teams/%s/admins/%s" % (team_name, user_name_3))
    )
    assert re.status_code == 200
    re = requests.get(api_v0("teams/%s/users" % team_name))
    assert re.status_code == 200
    ...
    re = requests.delete(
        api_v0(
            "teams/%s/rosters/%s/users/%s"
            % (team_name, roster_name, user_name_3)
        )
    )
    assert re.status_code == 200
    re = requests.get(api_v0("teams/%s/users" % team_name))
    ...
    re = requests.delete(
        api_v0("teams/%s/admins/%s" % (team_name, user_name_2))
    )
    assert re.status_code == 200
    re = requests.get(api_v0("teams/%s/users" % team_name))
    assert re.status_code == 200
    ...
    re = requests.delete(
        api_v0(
            "teams/%s/rosters/%s/users/%s"
            % (team_name, roster_name, user_name_2)
        )
    )
    assert re.status_code == 200
    re = requests.get(api_v0("teams/%s/users" % team_name))
...
def test_api_v0_team_summary(team, user, role, event):
    ...
    assert all(
        results["current"][role_name][0][key] == event_data_1[key]
        for key in keys
    )
    assert all(
        results["current"][role_name_2][0][key] == event_data_2[key]
        for key in keys
    )
    assert all(
        results["next"][role_name][0][key] == event_data_3[key] for key in keys
    )
    assert all(
        results["next"][role_name_2][0][key] == event_data_4[key]
        for key in keys
    )
...
def test_api_v0_team_override_number(team, user, role, event):
    ...
    re = requests.put(
        api_v0("teams/" + team_name),
        json={"override_phone_number": override_num},
    )
