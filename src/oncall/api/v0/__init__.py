...
def init(application, config):
    from . import team, team_changes, team_oncall, team_summary, teams
    ...
    from . import roster, roster_suggest, rosters

    application.add_route("/api/v0/teams/{team}/rosters", rosters)
    application.add_route("/api/v0/teams/{team}/rosters/{roster}", roster)
    application.add_route(
        "/api/v0/teams/{team}/rosters/{roster}/{role}/suggest", roster_suggest
    )

    from . import roster_user, roster_users

    application.add_route(
        "/api/v0/teams/{team}/rosters/{roster}/users", roster_users
    )
    application.add_route(
        "/api/v0/teams/{team}/rosters/{roster}/users/{user}", roster_user
    )

    from . import schedule, schedules

    application.add_route(
        "/api/v0/teams/{team}/rosters/{roster}/schedules", schedules
    )
    ...
    from . import service, service_oncall, services
    ...
    application.add_route(
        "/api/v0/services/{service}/oncall/{role}", service_oncall
    )

    from . import service_teams, team_service, team_services

    application.add_route("/api/v0/team_services", team_service)
    application.add_route("/api/v0/teams/{team}/services", team_services)
    application.add_route(
        "/api/v0/teams/{team}/services/{service}", team_service
    )
    ...
    from . import user, user_notifications, user_teams, users
    ...
    application.add_route(
        "/api/v0/notifications/{notification_id}", user_notification
    )

    from . import modes, notification_types
    ...
    application.add_route(
        "/api/v0/users/{user_name}/pinned_teams/{team_name}", user_pinned_team
    )
    ...
    application.add_route(
        "/api/v0/teams/{team}/subscriptions", team_subscriptions
    )
    application.add_route(
        "/api/v0/teams/{team}/subscriptions/{subscription}/{role}",
        team_subscription,
    )

    from . import team_ical, user_ical
    ...
    application.add_route(
        "/api/v0/ical_key/requester/{requester}", ical_key_requester
    )
    ...
    if iris.client and config.get("iris_plan_integration", {}).get("activated"):
        ...
        application.add_route(
            "/api/v0/teams/{team}/iris_escalate", team_iris_escalate
        )
