...
def on_put(req, resp, event_id):
    ...
    **Example request:**
    ...
    try:
        ...
        now = time.time()
        if (
            event_data["start"] < now - constants.GRACE_PERIOD
            or data["start"] < now - constants.GRACE_PERIOD
        ):
            # Make an exception for editing event end times
            if not (
                all(
                    event_data[key] == new_event[key]
        ...
        if not user_in_team_by_name(
            cursor, new_event["user"], event_data["team"]
        ):
            raise HTTPBadRequest(
        ...
        create_audit(
            {"old_event": event_data, "request_body": data},
            event_data["team"],
            EVENT_EDITED,
            req,
            cursor,
