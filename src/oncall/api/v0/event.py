...
def on_get(req, resp, event_id):
    ...
    with db.connect() as connection:
        ...
        if not data:
            raise HTTPNotFound(description=f"Event with ID {event_id} not found")
...
def on_put(req, resp, event_id):
    ...
    if "end" in data and "start" in data and data["start"] is not None and data["end"] is not None and data["start"] >= data["end"]:
    ...
    if invalid_cols:
         raise HTTPBadRequest("Invalid event update", f"Invalid column(s) provided: {', '.join(invalid_cols)}")
    ...
    for col, value in data.items():
        ...
        if col in update_columns:
            set_clause_snippets.append(update_columns[col]) # Add the snippet with %(name)s placeholder
            update_data_params[col] = value # Add the value to the parameters dict
    ...
    if not set_clause_snippets: # Should not happen with link_id = NULL always added, but defensive
    ...
    with db.connect() as connection:
        cursor = connection.cursor(db.DictCursor) # Use DictCursor for fetching event data
        ...
        if not event_data:
            raise HTTPNotFound(description=f"Event with ID {event_id} not found for update")

        # 2. Perform authorization checks
        check_calendar_auth(event_data["team"], req) # Check general calendar auth
        ...
        if is_past_event or is_new_start_in_past:
            ...
            is_only_extending_end_into_future = (
                event_data["start"] == new_start and # Start time is unchanged
                event_data["role_id"] == data.get("role_id", event_data["role_id"]) and # Role is unchanged (or same)
                event_data["user_id"] == data.get("user_id", event_data["user_id"]) and # User is unchanged (or same)
                new_end > now # New end time is in the future
            )
            ...
            if not is_only_extending_end_into_future:
                 ...
                 try:
                    check_team_auth(event_data["team"], req) # Check admin auth for the team
        ...
        if "user" in data: # Only check if user is being updated
             ...
             if not user_in_team_by_name(cursor, new_user_name, event_data["team"]):
                 # Raise exception within the with block
                 raise HTTPBadRequest(
                     "Invalid event update", f"New event user '{new_user_name}' must be part of team '{event_data['team']}'"
                 )
        ...
        try:
            ...
            audit_new_event_context = ", ".join(f"{key}: {value}" for key, value in data.items())
            create_audit(
                {"old_event": event_data, "request_body": data, "new_event_context": audit_new_event_context},
            ...
            if not new_ev_data: # Should not happen if update and previous select worked
                 raise HTTPError("500 Internal Server Error", "Database Error", f"Could not retrieve new info for event ID {event_id_int} after update")

            # Prepare context for notification
            notification_context = {
                "full_name": event_data["full_name"], # Original full name? Or fetch new one? Original code used old.
                "role": event_data["role"], # Original role name? Or fetch new one? Original code used old.
            ...
            original_start_time = event_data["start"] # Use original start time for notification context? Original code did.
...
def on_delete(req, resp, event_id):
    ...
    with db.connect() as connection:
        ...
        if not ev_info:
            ...
            raise HTTPNotFound(description=f"Event with ID {event_id} not found for deletion")

        try:
            # 2. Perform authorization checks
            check_calendar_auth(ev_info["team"], req) # Check general calendar auth
            ...
            cursor.execute("DELETE FROM `event` WHERE `id`=%s", (event_id_int,)) # Parameterize event_id
            ...
            create_audit({"old_event": ev_info}, ev["team"], EVENT_DELETED, req, cursor) # Use ev["team"] as original code
            ...
            print(f"Error during event delete transaction for event ID {event_id_int}: {e}") # Replace with logging
