...
def on_delete(req, resp, link_id):
    ...
    **Example request:**
    ...
    try:
        ...
        create_audit(
            {"old_event": data}, ev["team"], EVENT_DELETED, req, cursor
        )
        connection.commit()
    finally:
        cursor.close()
...
def on_put(req, resp, link_id):
    ...
    **Example request:**
    ...
    try:
        ...
        event_summary["start"] = min(event_data, key=itemgetter("start"))[
            "start"
        ]
        ...
        if event_summary["start"] < now - constants.GRACE_PERIOD:
            raise HTTPBadRequest(
                "Invalid event update", "Editing events in the past not allowed"
            )
        check_calendar_auth(event_summary["team"], req)

        update = (
            "UPDATE `event` SET %s WHERE link_id = %%(link_id)s" % update_cols
        )
        ...
        if cursor.rowcount == 0:
            raise HTTPNotFound()
        create_audit(
            {"old_event": event_summary, "request_body": data},
            event_summary["team"],
            EVENT_EDITED,
            req,
            cursor,
        ...
        cursor.execute(
            "SELECT `user_id`, role_id FROM `event` WHERE `link_id` = %s",
            link_id,
        )
        ...
        create_notification(
            ...
            {event_summary["role_id"], new_ev["role_id"]},
            EVENT_EDITED,
            {event_summary["user_id"], new_ev["user_id"]},
            cursor,
