    ...
    try:
        ...
        for idx, e in enumerate(events):
            ...
            if start <= e["start"] and end >= e["end"]:
                ...
                raise HTTPBadRequest(
                    "Invalid override request",
                    "events must overlap with override time range",
                )
        ...
        if edit_start:
            ...
            cursor.execute(
                "UPDATE `event` SET `start` = %s WHERE `id` IN %s", (end, ids)
            )
        if edit_end:
            ids = [e["id"] for e in edit_end]
            cursor.execute(
                "UPDATE `event` SET `end` = %s WHERE `id` IN %s", (start, ids)
            )
        if delete:
        ...
        context = {
            "full_name_0": full_names[user_id],
            "full_name_1": full_names[events[0]["user_id"]],
            "role": ret_data[0]["role"],
            "team": ret_data[0]["team"],
        }
        create_notification(
            context,
            events[0]["team_id"],
        ...
        create_audit(
            {"new_events": ret_data, "request_body": data},
            ret_data[0]["team"],
            EVENT_SUBSTITUTED,
            req,
            cursor,
