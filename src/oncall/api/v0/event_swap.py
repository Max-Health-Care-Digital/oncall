...
from ...utils import create_audit, create_notification, load_json_body
    ...
    **Example request**:
    ...
    try:
        ...
        raise HTTPBadRequest(
            "Invalid event swap request", "Must provide 2 events"
        )
    ...
    try:
        ...
        if any([ev["start"] < now - constants.GRACE_PERIOD for ev in events]):
            raise HTTPBadRequest(
                "Invalid event swap request", "Cannot edit events in the past"
            )
        ...
        cursor.execute(
            change_queries[0], (user_1, [e0["id"] for e0 in events_0])
        )
        cursor.execute(
            change_queries[1], (user_0, [e1["id"] for e1 in events_1])
        )

        cursor.execute(
            "SELECT id, full_name FROM user WHERE id IN %s", ([user_0, user_1],)
        )
        full_names = {row["id"]: row["full_name"] for row in cursor}
        cursor.execute(
            "SELECT name FROM team WHERE id = %s", events[0]["team_id"]
        )
