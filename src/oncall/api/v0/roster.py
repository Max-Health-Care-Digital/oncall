...
def on_get(req, resp, team, roster):
    ...
    **Example response**:
    ...
    with db.connect() as connection:
        ...
        if not results:
            raise HTTPNotFound(
                description=f"Roster '{roster_name}' not found for team '{team_name}'"
            )
        ...
        schedules = get_schedules(
            {"roster_id": roster_id}, dbinfo=(connection, cursor)
        )  # Filter by roster_id here
...
def on_put(req, resp, team, roster):
    ...
    **Example request:**
    ...
    roster_order_list = data.get(
        "roster_order"
    )  # Renamed to clarify it's a list
    ...
    if not (new_roster_name is not None or roster_order_list is not None):
        ...
        raise HTTPBadRequest(
            ...
            "Missing roster name or roster_order in request body",
        )

    if (
        new_roster_name is not None and new_roster_name == ""
    ):  # Check for empty string if name provided
        raise HTTPBadRequest(
            "Invalid roster name", "Empty roster name provided"
        )
    ...
    with db.connect() as connection:
        ...
        if not ids_info:
            raise HTTPNotFound(
                description=f"Roster '{roster_name}' not found for team '{team_name}' for update"
            )
        ...
        try:
            ...
            if roster_order_list is not None:
                ...
                if not isinstance(roster_order_list, list):
                    raise HTTPBadRequest(
                        "Invalid roster order", "roster_order must be a list"
                    )
                ...
                current_roster_users = {
                    row[0] for row in cursor.fetchall()
                }  # Fetch all user names

                # Validate provided roster_order_list against current users
                if not all(
                    user in current_roster_users for user in roster_order_list
                ):
                    raise HTTPBadRequest(
                ...
                order_update_params = [
                    (
                        ...
                        roster_id,
                        user,
                    )  # Update roster_priority, WHERE roster_id, WHERE user_name
            ...
            if new_roster_name is not None and new_roster_name != roster_name:
                ...
                invalid_char = invalid_char_reg.search(
                    new_roster_name
                ...
                cursor.execute(
                    ...
                    (
                        new_roster_name,
                        roster_id,
                    ),  # Parameterize new_roster_name and roster_id
                )

                # Create audit trail entry for name change
                create_audit(
                    {
                        "old_name": roster_name,
                        "new_name": new_roster_name,
                    },  # Use original and new names
    ...
    resp.status = (
        HTTP_204  # Standard response for successful PUT with no response body
    )
...
def on_delete(req, resp, team, roster):
    ...
    with db.connect() as connection:
        ...
        cursor.execute(
            "SELECT r.id, t.id FROM roster r JOIN team t ON r.team_id = t.id WHERE r.name = %s AND t.name = %s",
        ...
        if not roster_team_ids:
            ...
            raise HTTPNotFound(
                description=f"Roster '{roster_name}' not found for team '{team_name}' for deletion"
            )
        ...
        try:
            ...
            cursor.execute(
                "SELECT user_id FROM roster_user WHERE roster_id = %s",
                (roster_id,),
            )
            user_ids_in_roster = [
                row[0] for row in cursor.fetchall()
            ...
            cursor.execute(
                "DELETE FROM roster_user WHERE roster_id = %s", (roster_id,)
            )
            ...
            if user_ids_in_roster:
                ...
                cursor.execute(
                    ...
                    (user_ids_in_roster_tuple, team_id, team_id, team_id),
                )
            ...
            roster_deleted_count = (
                cursor.rowcount
            )  # Should be 1 if deletion by ID worked
            ...
            create_audit(
                {"name": roster_name}, team_name, ROSTER_DELETED, req, cursor
            )  # Use variable names, pass cursor
            ...
            Exception
        ) as e:  # Catch potential exceptions during the transaction
            # The with statement handles rollback automatically if an exception is raised within the block before commit.
            print(
                f"Error during roster delete transaction for {team_name}/{roster_name}: {e}"
