...
from falcon import HTTPBadRequest, HTTPForbidden, HTTPNotFound
    ...
    **Example request:**
    ...
    if "roster" in data and "team" not in data:
        raise HTTPBadRequest(
            "Invalid edit", "team must be specified with roster"
        )
    cols = ", ".join(columns[col] for col in data)

    update = (
        "UPDATE `schedule` SET " + cols + " WHERE `id`=%d" % int(schedule_id)
    )
    ...
    if events:
        ...
        cursor.execute(
            "SELECT duration FROM schedule_event WHERE schedule_id = %s",
            schedule_id,
        )
    ...
    if advanced_mode is None:
        cursor.execute(
            "SELECT advanced_mode FROM schedule WHERE id = %s", schedule_id
        )
    ...
    if not advanced_mode and not simple:
        raise HTTPBadRequest(
            "Invalid edit", "schedule cannot be represented in simple mode"
        )
    ...
    if events:
        cursor.execute(
            "DELETE FROM `schedule_event` WHERE `schedule_id` = %s", schedule_id
        )
    ...
    if scheduler and scheduler["name"] == "round-robin":
        ...
        cursor.execute(
            "DELETE FROM `schedule_order` WHERE `schedule_id` = %s", schedule_id
        )
        cursor.executemany(
