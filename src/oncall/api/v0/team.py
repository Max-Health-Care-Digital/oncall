...
def populate_team_users(cursor, team_dict):
    ...
    team_dict["users"] = dict(
        (r["name"], get_user_data(None, {"name__eq": r["name"]})[0])
        for r in cursor
    )
    ...
    **Example request:**
    ...
    if "scheduling_timezone" in data:
        if data["scheduling_timezone"] not in SUPPORTED_TIMEZONES:
            raise HTTPBadRequest(
                "invalid payload",
                "requested scheduling_timezone is not supported. Supported timezones: %s"
                % str(SUPPORTED_TIMEZONES),
            )
...
def on_delete(req, resp, team):
    ...
    **Example request:**
    ...
    cursor.execute(
        "UPDATE `team` SET `name` = %s WHERE `name`= %s", (new_team, team)
    )
    cursor.execute(
        "INSERT INTO `deleted_team` (team_id, new_name, old_name, deletion_date) VALUES (%s, %s, %s, %s)",
        (team_id, new_team, team, deletion_date),
    )
    connection.commit()
    cursor.close()
    connection.close()
