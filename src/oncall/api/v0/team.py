...
cols = {"name", "description", "slack_channel", "slack_channel_notifications", "email", "scheduling_timezone",
...
def populate_team_users(cursor, team_dict):
    ...
    team_dict["users"] = dict(
        (r["name"], get_user_data(None, {"name__eq": r["name"]})[0]) # get_user_data called here
        for r in cursor
    )
...
def on_get(req, resp, team):
    ...
    """
    team_name = unquote(team) # Renamed variable to avoid shadowing function name
    ...
    with db.connect() as connection:
        ...
        if not results:
            raise HTTPNotFound(description=f"Team '{team_name}' not found or not active")
...
def on_put(req, resp, team):
    ...
    if "iris_plan" in data and data["iris_plan"] and iris.client is not None:
        ...
        try:
            plan_resp = iris.client.get(
                 iris.client.url + "plans?name=%s&active=1" % iris_plan # UNSAFE string formatting
            )
    ...
    if "iris_enabled" in data:
        if not isinstance(data["iris_enabled"], bool): # Use isinstance for type checking
            raise HTTPBadRequest(
    ...
    for d in data_cols:
        if d in cols: # Only include valid columns for update
            set_clause_snippets.append(f"`{d}`=%s") # Use f-string for snippet, %s placeholder
    ...
    with db.connect() as connection:
        ...
        try:
            ...
            cursor.execute(update_query, tuple(query_params)) # Convert list to tuple for execute
            ...
            if cursor.rowcount == 0:
                 ...
                 raise HTTPNotFound(description=f"Team '{team_name}' not found for update")
            ...
            create_audit({"request_body": data}, team_name, TEAM_EDITED, req, cursor)
...
def on_delete(req, resp, team):
    ...
    with db.connect() as connection:
        ...
        if deleted_row_count == 0:
            ...
            raise HTTPNotFound(description=f"Team '{team_name}' not found for deletion")
        ...
        if not team_id_result: # Should not happen if deleted_row_count > 0, but defensive check
             raise HTTPError("500 Internal Server Error", "Database Error", f"Could not retrieve ID for deleted team '{team_name}'")
        team_id = team_id_result[0]


        # Change name in team table to preserve a clean namespace
        cursor.execute(
            "UPDATE `team` SET `name` = %s WHERE `name`= %s", (new_team_name, team_name)
        )
