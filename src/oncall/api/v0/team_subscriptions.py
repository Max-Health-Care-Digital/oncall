...
def on_get(req, resp, team):
    ...
    cursor.execute(
        """SELECT `subscription`.`name` AS `subscription`, `role`.`name` AS `role` FROM `team`
...
def on_post(req, resp, team):
    ...
    if not sub_name or not role_name:
        raise HTTPBadRequest(
            "Invalid subscription", "Missing subscription name or role name"
        )
    if sub_name == team:
        raise HTTPBadRequest(
            "Invalid subscription",
            "Subscription team must be different from subscribing team",
        )
    connection = db.connect()
