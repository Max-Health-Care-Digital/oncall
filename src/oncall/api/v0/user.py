...
from falcon import HTTP_204, HTTPBadRequest, HTTPNotFound, HTTPError
...
def on_put(req, resp, user_name):
    ...
    **Example request:**
    ...
    for field, value in data.items(): # Iterate through items to get key and value
        if field == "contacts":
            set_contacts = True
    ...
    with db.connect() as connection:
        ...
        if set_clause:
            ...
            if cursor.rowcount == 0: # Changed check from != 1 to == 0 for clarity with UPDATE
            ...
            if cursor.rowcount != 1:
                 raise HTTPBadRequest(
                     "Update Failed", "Could not update user (check name or if multiple users exist)"
                 )
        ...
        if set_contacts:
            ...
            if not isinstance(data["contacts"], dict):
                 raise HTTPBadRequest("Invalid contacts data", "Contacts must be a dictionary")

            for mode, dest in data["contacts"].items():
                contact = {}
                contact["mode"] = mode
                contact["destination"] = dest
                contact["user"] = user_name # Use the original user_name for lookup
                contacts_data_for_executemany.append(contact)

            if contacts_data_for_executemany: # Only execute if there are contacts to update
                 try:
                     cursor.executemany(contacts_query, contacts_data_for_executemany)
                 except db.IntegrityError as e:
