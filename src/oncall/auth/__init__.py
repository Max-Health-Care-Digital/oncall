...
def is_god(challenger):
    ...
    cursor.execute(
        "SELECT `id` FROM `user` WHERE `god` = TRUE AND `name` = %s", challenger
    )
...
def check_user_auth(user, req):
    ...
    if user_in_query != 0 or is_god(challenger):
        return
    raise HTTPForbidden(
        "Unauthorized", 'Action not allowed for "%s"' % challenger
    )
...
def check_team_auth(team, req):
    ...
    raise HTTPForbidden(
        "Unauthorized",
        'Action not allowed: "%s" is not an admin for "%s"'
        % (challenger, team),
    )
...
def check_calendar_auth(team, req, user=None):
    ...
    raise HTTPForbidden(
        "Unauthorized",
        'Action not allowed: "%s" is not part of "%s"' % (challenger, team),
    )
...
def check_calendar_auth_by_id(team_id, req):
    ...
    raise HTTPForbidden(
        "Unauthorized",
        'Action not allowed: "%s" is not a team member' % (challenger),
    )
...
def authenticate_application(auth_token, req):
    if not auth_token.startswith("hmac "):
        raise HTTPUnauthorized(
            "Authentication failure", "Invalid digest format", ""
        )
    method = req.method
    ...
    try:
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT `key` FROM `application` WHERE `name` = %s", app_name
        )
        if cursor.rowcount > 0:
            ...
            if is_client_digest_valid(
                ...
                return
            elif is_client_digest_valid(
                client_digest, api_key, window - 1, method, path, body
            ):
                req.context["app"] = app_name
                return
            elif is_client_digest_valid(
                client_digest, api_key, long_window, method, path, body
            ):
                req.context["app"] = app_name
                return
            elif is_client_digest_valid(
                client_digest, api_key, long_window - 1, method, path, body
            ):
                req.context["app"] = app_name
                return
            else:
                raise HTTPUnauthorized(
                    "Authentication failure", "Wrong digest", ""
                )
        else:
            cursor.close()
            connection.close()
            raise HTTPUnauthorized(
                "Authentication failure", "Application not found", ""
            )
...
def _authenticate_user(req):
    ...
    try:
        ...
        cursor.execute(
            "SELECT `csrf_token` FROM `session` WHERE `id` = %s", session["_id"]
        )
        ...
        if req.get_header("X-CSRF-TOKEN") != token:
            ...
            connection.close()
            raise HTTPUnauthorized(
                "Invalid Session", "CSRF validation failed", ""
            )
