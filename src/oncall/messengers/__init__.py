...
def init_messengers(messengers):
    for messenger in messengers:
        ...
        instance = getattr(
            importlib.import_module(module_path), messenger["type"]
        )(messenger)
...
def send_message(message):
    for messenger in _active_messengers[message["mode"]]:
        logger.debug(
            "Attempting %s send using messenger %s", message["mode"], messenger
        )
        try:
            return messenger.send(message)
        except Exception:
            logger.exception(
                "Sending %s with messenger %s failed", message, messenger
            )
            continue

    raise OncallMessengerException(
        "All %s messengers failed for %s" % (message["mode"], message)
    )
