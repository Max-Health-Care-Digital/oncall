...
class Scheduler(object):
    ...
    def get_schedule_last_event_end(self, schedule, cursor, table_name="event"):
        query = (
            "SELECT `end` FROM `%s` WHERE `schedule_id` = %%r ORDER BY `end` DESC LIMIT 1"
            % table_name
        )
    ...
    def get_schedule_last_epoch(self, schedule, cursor):
        cursor.execute(
            "SELECT `last_epoch_scheduled` FROM `schedule` WHERE `id` = %s",
            schedule["id"],
        )
        if cursor.rowcount != 0:
    ...
    def get_busy_user_by_event_range(
        self, user_ids, team_id, events, cursor, table_name="event"
    ):
    ...
    def find_least_active_user_id_by_team(
        self, user_ids, team_id, start_time, role_id, cursor, table_name="event"
    ):
        """
        ...
        if cursor.rowcount != 0:
            ...
            return min(cursor.fetchall(), key=operator.itemgetter("last_end"))[
                "user_id"
            ]
        else:
            return None

    def find_new_user_in_roster(
        self,
        roster_id,
        team_id,
        start_time,
    ...
    def create_events(
        ...
        if len(events) == 1:
            ...
            context = {"team": team_id, "role": role_id, "full_name": name}
            create_notification(
                context,
                team_id,
                [role_id],
    ...
    def set_last_epoch(self, schedule_id, last_epoch, cursor):
        cursor.execute(
            "UPDATE `schedule` SET `last_epoch_scheduled` = %s WHERE `id` = %s",
    ...
    def get_closest_epoch(self, dt):
        ...
        before = before_sunday.replace(
            hour=0, minute=0, second=0, microsecond=0, tzinfo=None
        )

        after_sunday = dt + timedelta(days=7 - dt.isoweekday())
        after = after_sunday.replace(
            hour=0, minute=0, second=0, microsecond=0, tzinfo=None
        )
    ...
    def calculate_future_events(self, schedule, cursor, start_epoch=None):
        ...
        if start_epoch is None:
            last_epoch_timestamp = self.get_schedule_last_epoch(
                schedule, cursor
            )

            # Handle new schedules, start scheduling from current week
            if last_epoch_timestamp is None:
                start_dt = datetime.fromtimestamp(time.time(), utc).astimezone(
                ...
                last_epoch_dt = datetime.fromtimestamp(
                    last_epoch_timestamp, utc
                )
                localized_last_epoch = last_epoch_dt.astimezone(
                    timezone(schedule["timezone"])
                )
                next_epoch = self.get_closest_epoch(
                    localized_last_epoch
                ) + timedelta(days=7 * period)
        else:
            next_epoch = start_epoch

        cutoff_date = datetime.fromtimestamp(time.time(), utc) + timedelta(
            days=schedule["auto_populate_threshold"]
        )
        cutoff_date = cutoff_date.replace(tzinfo=None)
        future_events = []
        # Start scheduling from the next epoch
        while cutoff_date > next_epoch:
            epoch_events = self.generate_events(
        ...
        return future_events, self.utc_from_naive_date(
            next_epoch - timedelta(days=7 * period), schedule
        )

    def find_next_user_id(
        self, schedule, future_events, cursor, table_name="event"
    ):
        ...
        for uid in self.get_busy_user_by_event_range(
            user_ids, team_id, future_events, cursor, table_name
        ):
            user_ids.remove(uid)
        if not user_ids:
            logger.info("All users have conflicting events, skipping...")
            return None
        new_user_ids = self.find_new_user_in_roster(
            roster_id, team_id, start, role_id, cursor, table_name
        )
        available_and_new = new_user_ids & user_ids
        if available_and_new:
            logger.info(
                "Picking new and available user from %s", available_and_new
            )
            return available_and_new.pop()

        logger.debug("picking user between: %s, team: %s", user_ids, team_id)
        return self.find_least_active_user_id_by_team(
            user_ids, team_id, start, role_id, cursor, table_name
        )
    ...
    def schedule(self, team, schedules, dbinfo):
        ...
        for schedule in schedules:
            ...
            future_events, last_epoch = self.calculate_future_events(
                schedule, cursor
            )
        ...
        for schedule, epoch in sorted(
            events,
            key=lambda x: (min(ev["start"] for ev in x[1]), x[0]["role_id"]),
        ):
    ...
    def build_preview_response(
        self, cursor, start__lt, end__ge, team__eq, table_name="temp_event"
    ):
        """
        Builds the JSON response for a schedule preview by querying the temporary table.
        """
        where_clause = []
        where_vals = []

        # Build WHERE clause dynamically based on provided constraints
        if start__lt is not None:
            where_clause.append(constraints["start__lt"])
            where_vals.append(start__lt)
        if end__ge is not None:
            where_clause.append(constraints["end__ge"])
            where_vals.append(end__ge)
        if team__eq is not None:
            where_clause.append(constraints["team__eq"])
        ...
        query = f"""
                SELECT {all_columns} FROM `{table_name}` AS `temp_event`
                JOIN `user` ON `user`.`id` = `temp_event`.`user_id`
                JOIN `team` ON `team`.`id` = `temp_event`.`team_id`
                JOIN `role` ON `role`.`id` = `temp_event`.`role_id`
    ...
    def populate(self, schedule, start_time, dbinfo, table_name="event"):
        ...
        if handoff < utc.localize(datetime.utcnow()):
            ...
            raise HTTPBadRequest(
                "Invalid populate/preview request",
                "cannot populate/preview starting in the past",
            )
        ...
        future_events = [
            [x for x in evs if x["start"] >= start_time]
            for evs in future_events
        ]
        future_events = [x for x in future_events if x != []]
        if future_events:
            first_event_start = min(future_events[0], key=lambda x: x["start"])[
                "start"
            ]
            query = (
                "DELETE FROM %s WHERE schedule_id = %%s AND start >= %%s"
                % table_name
            )
        ...
        # Create events in the db, associating a user to them
        for epoch in future_events:
            user_id = self.find_next_user_id(
                schedule, epoch, cursor, table_name
            )
            if not user_id:
                continue
            self.create_events(
