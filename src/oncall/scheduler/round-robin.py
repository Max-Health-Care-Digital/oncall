...
class Scheduler(default.Scheduler):
    ...
    def guess_last_scheduled_user(
        self, schedule, start, roster, cursor, table_name="event"
    ):
        query = (
            """
        ...
        cursor.execute(
            query, (schedule["team_id"], roster, start, schedule["role_id"])
        )
    ...
    def find_next_user_id(
        ...
        cursor.execute(
            """SELECT `user_id` FROM `roster_user`
        ...
        roster = [
            row["user_id"] for row in cursor if row["user_id"] in roster_users
        ]
        cursor.execute(
            "SELECT last_scheduled_user_id FROM schedule WHERE id = %s",
            schedule["id"],
        )
        ...
        if last_user not in roster:
            ...
            last_user = self.guess_last_scheduled_user(
                schedule, start, roster, cursor, table_name
            )
    ...
    def create_events(
        ...
        cursor.execute(
            "UPDATE `schedule` SET `last_scheduled_user_id` = %s WHERE `id` = %s",
            (user_id, schedule_id),
        )
    ...
    def populate(self, schedule, start_time, dbinfo, table_name="event"):
        ...
        cursor.execute(
            "UPDATE `schedule` SET `last_scheduled_user_id` = NULL WHERE `id` = %s",
            schedule["id"],
        )
        super(Scheduler, self).populate(
            schedule, start_time, dbinfo, table_name=table_name
        )
