...
def get_predefined_users(config):
    ...
    for user in config_users:
        ...
        for key in ["sms", "call"]:
            try:
                users[user["name"]][key] = normalize_phone_number(
                    users[user["name"]][key]
                )
...
def prune_user(engine, username):
    ...
    # The user has messages or some other user data which should be preserved. Just mark as inactive.
        ...
        engine.execute(
            "UPDATE `user` SET `active` = FALSE WHERE `name` = %s", username
        )
    ...
    try:
        engine.execute(
            "DELETE FROM `ical_key` WHERE `requester` = %s", username
        )
        logger.info("Invalidated ical_key of inactive user %s", username)
    except Exception as e:
        logger.error(
            "Invalidating ical_key of inactive user %s failed: %s", username, e
        )
        stats["sql_errors"] += 1
...
def fetch_ldap():
    ...
    while True:
        msgid = l.search_ext(
            base, ldap.SCOPE_SUBTREE, query, attrs, serverctrls=[req_ctrl]
        )
            ...
            if mode in ldap_contacts and ldap_contacts[mode]:
                ...
                if mode in db_contacts:
                    if ldap_contacts[mode] != db_contacts[mode]:
                        logger.debug(
                            "\tupdating %s (%s -> %s)"
                            % (mode, db_contacts[mode], ldap_contacts[mode])
                        )
                        engine.execute(
                            contact_update_sql,
                            (ldap_contacts[mode], username, modes[mode]),
                        )
                    ...
                    engine.execute(
                        contact_insert_sql,
                        (username, modes[mode], ldap_contacts[mode]),
                    )
                    stats["user_contacts_updated"] += 1
...
def sync(config, engine):
    ...
    if inactive_users:
        rows = engine.execute(
            "SELECT name FROM user WHERE active = TRUE AND name IN %s",
            inactive_users,
        )
    ...
    rows = engine.execute(
        "SELECT name FROM user WHERE active = FALSE AND name IN %s",
        ldap_usernames,
    )
    ...
    # get objects needed for insertion
    modes = dict(
        list(session.execute("SELECT `name`, `id` FROM `contact_mode`"))
    )
    ...
    for username in users_to_insert:
        ...
        try:
            ...
            user_id = engine.execute(
                user_add_sql, (username, full_name, photo_url)
            ).lastrowid
        except SQLAlchemyError:
            stats["users_failed_to_add"] += 1
            stats["sql_errors"] += 1
    ...
    for username in users_to_update:
        ...
        try:
            ...
            if full_name != db_contacts.get("full_name"):
                engine.execute(name_update_sql, (full_name, username))
                stats["user_names_updated"] += 1
            if "image_url" in LDAP_SETTINGS and not db_contacts.get(
                "photo_url"
            ):
                photo_url_tpl = LDAP_SETTINGS.get("image_url")
                photo_url = photo_url_tpl % username if photo_url_tpl else None
    ...
    for username in users_to_reactivate:
        ...
        try:
            engine.execute(
                "UPDATE user SET active = TRUE WHERE name = %s", username
            )
