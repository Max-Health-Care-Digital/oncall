...
def create_notification(
    context, team_id, role_ids, type_name, users_involved, cursor, **kwargs
):
    """
    ...
    """
    cursor.execute(
        """SELECT `user_id`, `mode_id`, `type_id`, `user`.`time_zone` FROM notification_setting
    ...
    for notification in notifications:
        ...
        for var_name, timestamp in kwargs.items():
            context[var_name] = " ".join(
                [
                    datetime.fromtimestamp(timestamp, timezone(tz)).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    tz,
                ]
        ...
        cursor.execute(
            """INSERT INTO `notification_queue` (`user_id`, `send_time`, `mode_id`, `context`, `type_id`,
                              `active`)
...
def subscribe_notifications(team, user, cursor):
    cursor.execute(
        "SELECT id FROM notification_setting WHERE user_id = (SELECT id FROM user WHERE name = %s) "
    ...
    if cursor.rowcount == 0:
        for time in constants.DEFAULT_TIMES:
            for mode in constants.DEFAULT_MODES:
                cursor.execute(
                    """INSERT INTO `notification_setting` (`user_id`, `team_id`, `mode_id`,
                                                                      `type_id`, `time_before`)
            ...
            query_vals = ", ".join(
                [
                    ...
                    % (setting_id, role)
                    for role in constants.DEFAULT_ROLES
                ]
            )
...
def unsubscribe_notifications(team, user, cursor):
    cursor.execute(
        "DELETE FROM notification_setting WHERE user_id = (SELECT id FROM user WHERE name = %s) "
        "AND team_id = (SELECT id FROM team WHERE name = %s)",
        (user, team),
    )
...
def load_json_body(req):
    try:
        ...
        raise HTTPBadRequest(
            "invalid JSON", "failed to decode json: %s" % str(e)
        )
