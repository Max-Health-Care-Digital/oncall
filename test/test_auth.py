...
def test_application_auth(mocker):
    ...
    re = client.simulate_get(
        "/dummy_path", headers={"AUTHORIZATION": "hmac dummy:abc"}
    )
    assert re.status_code == 401
    re = client.simulate_post(
        "/dummy_path",
        json={"example": "test"},
        headers={"AUTHORIZATION": "hmac dummy:abc"},
    )
    assert re.status_code == 401
    ...
    re = client.simulate_get(
        "/dummy_path", params={"abc": 123}, headers={"AUTHORIZATION": auth}
    )
    ...
    re = client.simulate_post(
        "/dummy_path", body=body, headers={"AUTHORIZATION": auth}
    )
