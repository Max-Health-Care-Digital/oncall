...
def test_find_new_user_as_least_active_user(mocker):
    ...
    mocker.patch(
        "oncall.scheduler.default.Scheduler.find_new_user_in_roster"
    ).return_value = {123}
    mocker.patch(
        "oncall.scheduler.default.Scheduler.get_roster_user_ids"
    ).return_value = {135, 123}
    mocker.patch(
        "oncall.scheduler.default.Scheduler.get_busy_user_by_event_range"
    )
    mocker.patch(
        "oncall.scheduler.default.Scheduler.find_least_active_user_id_by_team"
    )

    user_id = scheduler.find_next_user_id(
        MOCK_SCHEDULE, [{"start": 0, "end": 5}], None
    )
    assert user_id == 123
...
def test_calculate_future_events_7_24_shifts(mocker):
    mocker.patch(
        "oncall.scheduler.default.Scheduler.get_schedule_last_epoch"
    ).return_value = None
...
def test_existing_schedule(mocker):
    mock_dt = datetime.datetime(
        year=2017, month=2, day=5, hour=0, tzinfo=timezone("US/Pacific")
    )
    mocker.patch(
        "oncall.scheduler.default.Scheduler.get_schedule_last_epoch"
    ).return_value = calendar.timegm(mock_dt.astimezone(utc).timetuple())
    mocker.patch("time.time").return_value = time.mktime(
        datetime.datetime(year=2017, month=2, day=7).timetuple()
    )
    start = DAY + 10 * HOUR + 30 * MIN  # Monday at 10:30 am
    schedule_foo = {
        "timezone": "US/Pacific",
...
def test_existing_schedule_change_epoch(mocker):
    mock_dt = datetime.datetime(
        year=2017, month=2, day=5, hour=0, tzinfo=timezone("US/Eastern")
    )
    mocker.patch(
        "oncall.scheduler.default.Scheduler.get_schedule_last_epoch"
    ).return_value = calendar.timegm(mock_dt.astimezone(utc).timetuple())
...
def test_find_least_active_available_user(mocker):
    ...
    mocker.patch(
        "oncall.scheduler.default.Scheduler.get_roster_user_ids"
    ).return_value = [i for i in mock_user_ids]
    mock_busy_user_by_range = mocker.patch(
        "oncall.scheduler.default.Scheduler.get_busy_user_by_event_range"
    )
    mock_active_user_by_team = mocker.patch(
        "oncall.scheduler.default.Scheduler.find_least_active_user_id_by_team"
    )

    def mock_busy_user_by_range_side_effect(
        user_ids, team_id, events, cursor, table_name="event"
    ):
    ...
    mock_active_user_by_team.assert_called_with(
        {456, 789}, 1, 440, 2, None, "event"
    )
...
def test_find_least_active_available_user_conflicts(mocker):
    ...
    scheduler = oncall.scheduler.default.Scheduler()
    assert (
        scheduler.find_next_user_id(
            MOCK_SCHEDULE, future_events, None, table_name="event"
        )
        is None
    )
